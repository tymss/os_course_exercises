...
寄存器最快，完全硬件实现，空间小。高速缓存速度比内存快，需要硬件支持，空间也比较小。内存速度快于外存，但空间也远小于外存，所有程序运行都需要内存。外存空间最大，但速度比较慢，成本低。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

各层存储都处于繁忙状态，但又都能正常工作。
...
覆盖技术指按功能模块划分程序，不同时执行的模块可以共享一块内存区域相互覆盖使用。程序开发者的工作是对程序划分模块，确定模块之间的共享关系。

2. 什么是交换技术？覆盖与交换有什么不同？

交换技术将程序进程整个换入换出。覆盖是将程序划分，而交换换的是整个程序进程。
...
时间局部性是一条指令的一次执行和下次执行，一个数据的一次访问和下次访问都集中在一个较短时期内。空间局部性是指当前指令和邻近的几条指令，当前访问的数据和邻近的几个数据都集中在一个较小区域内。分支局部性是指一条跳转指令的两次执行，很可能跳到相同的内存位置。
...
虚拟存储是指只把当前指令执行所需要的部分进程地址空间内容放在物理内存中。虚拟存储与覆盖的区别是不需要程序员来划分程序模块，完全由操作系统自己执行，与交换的区别是它操作的是部分程序，而不是整个程序。好处是节省了内存，提供了大的虚拟空间，挑战是实现合适的页面调度算法比较复杂。

2. 虚拟存储需要什么样的支持技术？

硬件需要提供地址转换的支持，软件需要实现高效的调度算法。

### 8.5 虚拟页式存储

 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
 
 在页式存储系统的基础上，实现请求调页和页面置换。缺页中断处理的功能是从物理内存中将相应的页面调入内存。

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？

页表项需要增加控制位，比如驻留位、修改位、访问位、保护位等。

 2. 页式存储和虚拟页式存储的区别是什么？
 
 页式存储是内存中的不连续内存存储策略，而虚拟页式存储是内存外存间交换的技术。虚拟页式存储比页式存储多了调页请求和页面置换。

### 8.6 缺页异常

1. 缺页异常的处理流程？

缺页异常后，在外存中查找需要访问的物理页帧；若内存中的页面修改过，需要备份；修改内存内页表驻留位为0；页面换入；页表项驻留位修改为1；重新执行导致缺页异常的指令；
